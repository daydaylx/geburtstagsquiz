...
export function HostLobbyStage({
  ...
  const players = s.lobby?.players ?? [];
  const hasPlayers = players.length > 0;

  return (
    <div className="host-lobby-layout">
      <div className="host-lobby-info-bar">
        <div className="host-lobby-join-card">
          {s.qrCodeDataUrl && (
            <div className="host-lobby-info-qr">
              <img alt="Join QR" src={s.qrCodeDataUrl} />
            </div>
          )}
          <div className="host-lobby-join-copy">
            <span className="host-control-label">Beitritt</span>
            <div className="host-join-code-row">
        ...
        <div className="host-lobby-info-display" data-connected={s.displayConnected}>
          <span className="host-control-label">Display</span>
          <strong>{s.displayConnected ? "Verbunden" : "Nicht verbunden"}</strong>
          <small>{s.displayConnected ? "TV ist gekoppelt" : "Display-Fenster am TV öffnen"}</small>
      ...
      <div className="host-lobby-columns">
        <div className="host-lobby-players" data-empty={!hasPlayers ? "true" : undefined}>
          ...
          <div className="host-player-list">
            {!hasPlayers && (
              <div className="host-player-list-empty">
                <strong>Noch niemand beigetreten</strong>
                <span>Join-Code teilen und auf die erste Anmeldung warten.</span>
              </div>
            )}
            {players.map((p) => (
          ...
          <div className="host-lobby-extra-grid">
            ...
            <div className="host-card host-history-card">
