...
export function evaluateMajorityGuess(question: MajorityGuessQuestion, answers: SubmittedAnswer[]): RoundResult {
  ...
  const winningOptionIds =
    maxCount > 0
      ? question.options.map((option) => option.id).filter((optionId) => (counts.get(optionId) ?? 0) === maxCount)
      : [];
  ...
  const playerResults: PlayerRoundResult[] = answers.map((submittedAnswer) => {
    const isCorrect = submittedAnswer.answer.type === "option" && winningOptionIdSet.has(submittedAnswer.answer.value);
