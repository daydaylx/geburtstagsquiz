import type { OpenTextQuestion, PlayerRoundResult, RoundResult, SubmittedAnswer } from "@quiz/shared-types";
...
export function evaluateOpenText(question: OpenTextQuestion, answers: SubmittedAnswer[]): RoundResult {
  ...
  const playerResults: PlayerRoundResult[] = answers.map((submittedAnswer) => {
    const isCorrect =
      submittedAnswer.answer.type === "text" && acceptedAnswers.has(normalizeTextAnswer(submittedAnswer.answer.value));
    ...
    return {
      ...
      ...(submittedAnswer.answer.type === "text" ? { detail: { submittedText: submittedAnswer.answer.value } } : {}),
    };
  });
